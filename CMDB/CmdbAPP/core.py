...
class Asset(object):
    def __init__(self,request):
        ...
        self.field_sets = {
            'asset':['manufactory'],
            'server':['model','cpu_count','cpu_core_count','cpu_model',
                'raid_type','os_type','os_distribution','os_release'],
            'networkdevice':[]
        }
        self.response = {
            'error':[],
    ...
    def mandatory_check(self,data,only_check_sn=False):
        ...
        try:
            ...
            "Cannot find asset object in DB by using asset id [%s] and SN [%s] " % (data['asset_id'],
                                                                                    data['sn']))
            ...
            try:
                ...
                if self.mandatory_check(data,only_check_sn=True):
                    ...
                    if hasattr(self,'waiting_approval'):
                        response = {
                        'needs_aproval':
                        "this is a new asset,needs IT admin's approval to create the new asset id."}
    ...
    def reformat_components(self,identify_field,data_set):
        ...
        it uses key as identified field, the key is actually equals slot field in db model field,
        this unstandard
    ...
    def __update_asset_component(self,data_source,fk,update_fields,identify_field=None):
        ...
        identify_field: use this field to identify each component of an Asset ,
                    ...
                    if type(data_source) is list:
                        for source_data_item  in data_source:
                            ...
                            if key_field_data_from_source_data:
                                '''
                                find the matched source data for this component,then should compare each
                                field in this component to see if there's any changes since last update
                                '''
                                if key_field_data == key_field_data_from_source_data:
                                   ...
                                   '''
                                      must break ast last ,then if the loop is finished , logic will goes
                                      for else part,then you will know that no source data is matched for
                                      by using this key_field_data, that means , this item is lacked from
                                      source data,it makes sense when the hardware info got changed. e.g:
                                      one of the RAM is broken, sb takes it away,then this data will  not
                                      be reported in reporting data
                                   '''
                                   break
                            ...
                            '''couldn't find any matches, the asset component must be broken or
                            changed manually'''
                            self.response_msg("warning","AssetUpdateWarning",
                        ...
                        for key,source_data_item  in data_source.items():
                            ...
                            if key_field_data_from_source_data:
                                '''
                                find the matched source data for this component,then should compare
                                each field in this component to see if there's any changes since
                                last update
                                '''
                                if key_field_data == key_field_data_from_source_data:
                                   ...
                                   logic will goes for ..else part,then you will know that no source
                                   data is matched for by using this key_field_data, that means ,
                                   this item is lacked from source data, it makes sense when the
                                   hardware info got changed. e.g: one of the RAM is broken, sb takes
                                   it away,then this data will not be reported in reporting data
                                   '''
                                   break
    ...
    def __delete_components(self,all_components, delete_list , identify_field ):
        ...
        for i in deleting_obj_list:
            log_msg = \
                "Asset[%s] --> component[%s] --> is lacking from reporting source data,-- " \
                " assume it has been removed or replaced,will also delete it from DB" \
                %(self.asset_obj,i)
